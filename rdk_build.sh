...
CC_PATH=$RDK_PROJECT_ROOT_PATH/sys_mon_tools/iarm_event_sender
...
function build()
{
    ...
    if [ $RDK_PLATFORM_SOC = "intel" ]; then
       echo "building for ${RDK_PLATFORM_DEVICE}..."
        export TOOLCHAIN_DIR=$RDK_PROJECT_ROOT_PATH/sdk/toolchain/staging_dir
        export CROSS_TOOLCHAIN=$TOOLCHAIN_DIR
        export CROSS_COMPILE=$CROSS_TOOLCHAIN/bin/i686-cm-linux
        export CC=$CROSS_COMPILE-gcc
        export CXX=$CROSS_COMPILE-g++
        export OPENSOURCE_BASE=$RDK_PROJECT_ROOT_PATH/opensource
        ...
        export SDK_FSROOT=$RDK_PROJECT_ROOT_PATH/sdk/fsroot/ramdisk
	  ...
	  export GLIBS='-lglib-2.0 -lz'	  
	  FSROOT=$RDK_PROJECT_ROOT_PATH/sdk/fsroot
	  export IARM_PATH=$BUILDS_DIR/iarmbus
	...
	source $RDK_PROJECT_ROOT_PATH/sys_mon_tools/soc/stm/soc_env.sh
    fi
    if [ -d $IARM_PATH ]; then
	  export USE_IARM_BUS="y"
    else
          export IARM_PATH=$RDK_PROJECT_ROOT_PATH/iarm
	  export USE_IARM="y"
    fi	
